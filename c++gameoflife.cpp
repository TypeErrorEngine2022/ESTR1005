...
//  SDL window part, sturtures created by Emily Abrash on 11/15/16.]
//  Modified by Jacky Chen
...
void populateArray(int *lifeArray, int arLength)
{
    for (int i = 0; i < arLength; i++)
    {
        ...
        {
            lifeArray[i*arLength+j] = randNum(0, 10000000) % 2;	//Initialize each cell to be randomly alive or dead
        }
    }
    ...
    for (int i = 1; i < len - 1; i++)
    {
        for (int j = 1; j < len - 1; j++)
        {
            ...
            for (int dx = -1; dx <= 1; dx++)
            {
                ...
                {
                    sum += lifeArray[(i + dx) * len + (j + dy)];
                }
            }
            sum -= lifeArray[i * len + j];
            
            if (sum == 3)
            {
                myTempArray[i*len + j] = 1;	//It lives
            }
...
int main(int argc, const char * argv[]) {
    ...
    int seed = randNum(1, 10000);                        //Seed for random number generator
